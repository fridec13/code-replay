...
- 우측 타임라인 패널에서 **배속(0.1x ~ 5x, 스텝/초 기준)** 과 **이전/다음 스텝(◀ / ▶)** 으로 실행 흐름을 다시 볼 수 있습니다.
- 함수 호출이 **순번(#1, #2 …)** 과 함께 스피드런 구간(splits)처럼 나눠져 원하는 함수로 바로 점프할 수 있습니다.
- **변수 Diff 테이블**: 스텝마다 이전값 → 현재값 변화를 색상으로 한눈에 확인합니다. (선택) **문자/단어 단위 인라인 diff**로 바뀐 구간만 강조할 수 있습니다.
- **녹화 오류 표시**: 스크립트 오류 시에도 가능한 만큼 트레이스를 남기고, **에러 메시지**를 타임라인 패널과 알림에 표시합니다.
- **패널 UI**: Variables / Functions / 타임라인 / Console 영역 **높이·폭 조절**(드래그), 라이트·다크 테마에서 읽기 쉬운 **대비**를 맞춰 두었습니다.
...
| 정지 (처음으로) | **Stop** 버튼 |
| 배속 변경 | 패널 내 속도 버튼 (`0.1x` ~ `5x`) |
| 이전/다음 스텝 | **◀** / **▶** 버튼 (한 이벤트씩 이동) |
| 위치 이동 | 스크러버(슬라이더) 드래그 |
...
타임라인 위쪽에 현재 스텝의 변수 변화가 표시됩니다. 열 너비는 고정되고, 긴 값은 **줄바꿈**으로 표시됩니다.
...
**인라인 문자열 diff(선택)**  
설정에서 `codeReplay.varInlineDiff`를 `true`로 켜면, 이전 값과 현재 값이 둘 다 있을 때 **문자(char) 또는 단어(word)** 단위로 바뀐 구간만 굵게 표시합니다. 긴 문자열에서는 연산 비용이 커질 수 있어 기본값은 `false`입니다. 세부 단위는 `codeReplay.varDiffGranularity`(`char` | `word`)로 지정합니다.
...
```jsonc
{
  ...
  // 변수 표에서 문자열을 글자/단어 단위로 나눠 강조 (기본 false, 긴 값에서는 끄는 편이 빠름)
  "codeReplay.varInlineDiff": false,
  "codeReplay.varDiffGranularity": "char"
}
```
...
- 타임라인: 배속 **0.1x ~ 5x**, **◀ / ▶** 한 스텝 이동.
- 변수 Diff: 선택적 **문자/단어 인라인 diff** (`codeReplay.varInlineDiff`, `codeReplay.varDiffGranularity`).
- 녹화 실패 시에도 가능한 트레이스 유지 및 **에러 메시지** 표시.
- 패널 **리사이즈**, 라이트/다크 **가독성·대비** 개선.
